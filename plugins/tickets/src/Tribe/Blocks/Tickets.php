 ...
 * Tickets block Setup
 */
class Tribe__Gutenberg__Tickets__Blocks__Tickets
extends Tribe__Gutenberg__Common__Blocks__Abstract {
	...
	public function render_shortcode_attendees( $atts, $content = '' ) {
		...
		$content = tribe( 'gutenberg.tickets.template' )->template( 'blocks/tickets/registration/content', $args, false );
	...
	public function ticket_availability( $tickets = array() ) {
		...
		foreach ( $tickets as $ticket_id ) {
			...
			if ( 0 === $available ) {
				$response['tickets'][ $ticket_id ]['unavailable_html'] = tribe( 'gutenberg.tickets.template' )->template( 'blocks/tickets/quantity-unavailable', $ticket, false );
			}
		}
